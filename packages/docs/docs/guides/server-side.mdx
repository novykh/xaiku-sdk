...
The middleware manages visitor identity (GUID cookie) and can forward authenticated user context to the provider. Place it in your `middleware.js`:
...
2. Reads the `__xaiku__guid__` cookie from the request (or generates a new one).
3. Forwards `userId` and `orgId` to the server provider via request headers.
4. Sets the GUID cookie on the response so it persists across requests.

This ensures every visitor gets a stable GUID and user identity before any page renders. The `auth` callback is optional — without it, the middleware only manages the GUID cookie.
...
1. Reads `userId` and `orgId` from request headers set by the middleware (falls back to props if provided directly).
2. Reads the GUID from cookies (set by the middleware).
3. Creates a server SDK and fetches experiments via `sdk.getExperiments()`.
4. Passes `experiments`, `guid`, `userId`, `orgId`, and `pkey` to the client-side `XaikuProvider` from `@xaiku/react`.
...
| `userId` | `string` | -- | User ID. Automatically read from middleware when not provided. |
| `orgId` | `string` | -- | Organization ID. Automatically read from middleware when not provided. |
