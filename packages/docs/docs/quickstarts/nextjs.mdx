...
The middleware manages a GUID cookie for consistent variant assignment and can forward user identity to the provider.
...
The `auth` callback is optional. If provided, `userId` and `orgId` are forwarded to the provider automatically — no props needed in your layout.
